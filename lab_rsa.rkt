...
(define (mod-exp b e m)
  (cond [(zero? e) 1]
        [(and (> e 0) (zero? (modulo e 2))) (modulo (expt (mod-exp b (/ e 2) m) 2) m)]
        [(and (> e 0) (= 1 (modulo e 2))) (modulo (* b (modulo (mod-exp b (- e 1) m) m)) m)]
        ))
...
(define (encrypt plaintext key)
  (cond [(= 0 (length (string->list plaintext))) '()]
        [else (cons (mod-exp (char->integer (car (string->list plaintext))) (cadr key) (car key)) (encrypt (list->string (cdr (string->list plaintext))) key))]))
...
(define (helpme t m e)
  (cond [(= 0 (length t)) '()]
        [else (cons (integer->char (mod-exp (car t) e m)) (helpme (cdr t) m e)) ]))
