...
(define (pred p)
  (cond [(equal? p '(()) ) '()]
        [(null? p) (error-predecessor)]
        [else (list (pred (car p)))]))
...
(define (p-prime? p)
  (define (helper i n)
    (cond [(p> (p* i i) n) #t]
          [(p= (p% n i) z) #f]
          [else (helper (p+ i (succ(succ z))) n)]))
  
  (cond [(or (p> (succ z) p) (p= (succ z) p))  #f]
        [(p= p (succ(succ z))) #t]
        [(p= (p% p (succ(succ z))) z) #f]
        [else (helper (succ(succ(succ z))) p)]))
