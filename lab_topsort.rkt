...
;; 5 PTS
(define (find-top G)
  (if (null? (helpme G)) (topsort-error) (car (helpme G))))

(define (helpme G)
  (cond [(null? (get-vertices G)) '()]
        [(zero? (indegree (get-edges G) (car(get-vertices G)))) (cons (car(get-vertices G)) (helpme (make-graph (cdr(get-vertices G)) (get-edges G))))]
        [else (helpme (make-graph (cdr(get-vertices G)) (get-edges G)))]))
