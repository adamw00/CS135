...
(define (divides? a b)
  (cond [(and (zero? a) (zero? b)) #t]
        [(zero? b) #t]
        [(not (zero? a)) (if (zero? (modulo b a)) #t #f)]
        [else #f]))
...
(define (wj-solvable? cA cB g)
  (cond [(and (zero? (modulo g (gcd cA cB))) (or (>= cA g) (>= cB g))) #t]
        [else #f]))
...
(define (waterjug cA cB g)
  (wj-help 0 0 cA cB g))
(define (wj-help fA fB cA cB g)
  (cond [(= fB g) (list (list fA fB))]
        [(= fB cB) (cons (list fA fB) (wj-help fA 0 cA cB g))]
        [(= fA 0) (cons (list fA fB) (wj-help cA fB cA cB g))]
        [(> (+ fA fB) cB) (cons (list fA fB) (wj-help (- fA (- cB fB)) cB cA cB g))]
        [(<= (+ fA fB) cB) (cons (list fA fB) (wj-help 0 (+ fB fA) cA cB g))]))
...
(define (phelp n p)
    (cond [(zero? (modulo n p)) p]
          [else (phelp n (+ p 1))]))
(define (fhelp n)
  (cond [(= n 1) '()]
        [(= n 0) '(0)]
        [else (cons (phelp n 2) (fhelp (/ n (phelp n 2))))]))
...
;; 2 PTS
(define (prime? n)
  (if (and (= (length (factorize n)) 1) (> n 1)) #t #f))
...
;; 3 PTS
(define (cong-mod? m)
  (lambda (a b) (if (= (modulo a m) (modulo b m)) #t #f)))
