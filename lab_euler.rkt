...
(define (get-vertices G)
   (if (null? G) G (usort (cons(caar G)(cons(cadar G)(get-vertices (cdr G)))))))
...
(define (degree G v)
  (cond [(null? G) 0]
        [(and (equal? v (caar G)) (equal? v (cadar G))) (+ 2 (degree (cdr G) v))]
        [(or (equal? v (caar G))(equal? v (cadar G))) (+ 1 (degree (cdr G) v))]
        [else (degree (cdr G) v)]))
...
(define (evenHelper G V)
  (cond [(null? G) #t]
        [(null? V) #t]
        [(not (equal? 0 (modulo (degree G (car V)) 2))) #f]
        [else (evenHelper G (cdr V))]))
...
;; 4 PTS
(define (adjacent G v)
  (cond [(equal? G empty) empty]
        [(equal? (caar G) v) (usort (cons (cadar G)(adjacent (cdr G) v)))]
        [(equal? (cadar G) v) (usort (cons (caar G) (adjacent (cdr G) v)))]
...
;; 4 PTS
(define (connected? G)
  (if (null? G) #t (null? (remove(get-vertices G)(usort(dfs G (caar G)))))))
