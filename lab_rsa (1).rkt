...
(define (mod-exp b e m)
  (cond [(zero? e) 1]
        [(and (> e 0) (zero? (modulo e 2))) (modulo (expt (modulo (expt b (/ e 2)) m) 2) m)]
        [(and (> e 0) (= 1 (modulo e 2))) (modulo (* (modulo (expt b (- e 1)) m) b) m)]
        ))
...
(define (encrypt plaintext key)
  (cond [(= 0 (length (string->list plaintext))) '()]
        [else (cons (modulo (expt (char->integer (car (string->list plaintext))) (cadr key)) (car key)) (encrypt (list->string (cdr (string->list plaintext))) key))]))
...
(define (decrypt ciphertext key)
  (list->string (helpme ciphertext (car key) (cadr key))))

(define (helpme t m e)
  (cond [(= 0 (length t)) '()]
        [else (cons (integer->char (modulo (expt (car t) e) m)) (helpme (cdr t) m e)) ]))
