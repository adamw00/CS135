...
(define (compose S R)
  (cond [(null? S) '()]
        [(null? R) '()]
        [(equal? (caar S) (cadar R))
            (make-set(append (list(list(caar R) (cadar S))) (append (compose (cdr S) R) (compose S (cdr R)))))]
        [else  (append (compose (cdr S) R) (compose S (cdr R)))]))
...
(define (power k R)
  (cond [(equal? k 0)'()]
        [(equal? k 1) R]
        [else (make-set(compose(power(- k 1) R) R))]))
...
(define (transitive-closure R)
  (define (helpme R e)
    (cond [(zero? e) '()]
        [else (union (power e R) (helpme R (- e 1)))]))
  (helpme R (cardinality R)))
...
(define (equivalence? n R)
  (and (symmetric? R) (transitive? R) (reflexive? n R)))
...
(define (involutive? n R)
  (define (bijective? S a b)
    (cond [(and (null? a) (null? b) (null? S)) #t]
          [(null? S) #f]
          [(not (element? a (caar S))) #f]
          [(not (element? b (cadar S))) #f]
          [(and (element? a (caar S)) (element? b (cadar S))) (bijective? (cdr S) (remove a (caar S)) (remove b (cadar S)))]
          [else #t]))
  
  (define (set-to-n n)
    (cond [(= n 1) (list n)]
          [else (cons n (set-to-n (- n 1)))]))
  
  (and (symmetric? R) (bijective? R (set-to-n n) (set-to-n n))))
